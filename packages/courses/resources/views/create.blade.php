    ...
    <div class="card bcard">
        <div class="card-header bgc-primary-d1 text-white border-0 d-flex justify-content-between">
            <h4 class="text-120 mb-0">
                <span class="text-90">Crear curso</span>
            </h4>
            <img src="{{asset('img/flags/'.$language.'.png')}}" alt="{{ $language }}">
        </div>

        <form class="mt-lg-3" autocomplete="off" method="post" action="{{route('courses.store')}}" enctype="multipart/form-data">
            @csrf
            @livewire('form::input-text', ['name' => 'title', 'labelText' => 'Título', 'required'=>true])
            @livewire('utils::tinymce-editor', ['name' => 'text_info', 'labelText' => 'Información del curso', 'value' => old('text_info')])
            ...
            @if(config('bpanel4-courses.activate_inscriptions'))
                @livewire('form::input-checkbox', ['name' => 'inscription', 'value' => 1, 'labelText' => '¿Habilitar las inscripciones?', 'bpanelForm' => true, 'checked' => true])
            @endif
