...
namespace Bittacora\Courses\Models;
...
class CourseModel extends Model implements Sortable
{
    use HasFactory;
    use HasTranslations;
    use Userstamps;
    ...
    //Relación con las inscripciones de los cursos (si está en el proyecto)
    public function inscriptions()
    {
        if (class_exists('Bittacora\CourseInscriptions\Models\CourseInscriptionModel')) {
            return $this->hasMany(
                \Bittacora\CourseInscriptions\Models\CourseInscriptionModel::class,
                'course_id',   // 🔑 foreign key en course_inscriptions
                'id'           // 🔑 primary key en courses
            );
        }
    ...
    public function setMetas($request,$model){
        ...
        if(empty($request->input('meta_description'))){
            $description = $model->setMetaDescription($model->meta_description,$model->text);
            $model->fill([
                'meta_description' => $description,
            ]);
        }
        if(empty($request->input('meta_keywords'))){
            $keywords = $model->setMetaKeywords($model->meta_keywords,$model->text);
            $model->fill([
